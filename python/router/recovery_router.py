...
async def create(dto:RecoveryDto,session:Annotated[Session, Depends(get_session)]):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Error: {error_code}: {error_msg}")
    
    return JSONResponse(
            status_code=status.HTTP_200_OK,
            content={
                "status": {"status_code": status.HTTP_200_OK, "message": "Recovery token created"},
                "response": {},
            },
        )
